#!/bin/bash
# reorg_sim.sh - Geth Reorg Simulation for Docker (Linux)
# Auto-runs 3 nodes and provides IPC control for Node A
#
...
# IPC Sockets (Linux Standard Sockets)
IPC_A='/tmp/geth_A.ipc'
IPC_B='/tmp/geth_B.ipc'
IPC_C='/tmp/geth_C.ipc'
...
init_genesis() {
    ...
    mkdir -p "$DATA_DIR/A" "$DATA_DIR/B" "$DATA_DIR/C"
    
    log_info "Initializing genesis for Node A..."
    $GETH_BIN init --datadir "$DATA_DIR/A" "$TEMPLATE_DIR/genesis.json"
    log_info "Initializing genesis for Node B..."
    $GETH_BIN init --datadir "$DATA_DIR/B" "$TEMPLATE_DIR/genesis.json"
    log_info "Initializing genesis for Node C..."
    $GETH_BIN init --datadir "$DATA_DIR/C" "$TEMPLATE_DIR/genesis.json"
    
    log_info "Copying keystore to nodes..."
    cp -r "$TEMPLATE_DIR/keystore" "$DATA_DIR/A/"
    cp -r "$TEMPLATE_DIR/keystore" "$DATA_DIR/B/"
    cp -r "$TEMPLATE_DIR/keystore" "$DATA_DIR/C/"
    
    log_success "Genesis block initialized and keystore copied for Node A, B, and C"
}

clean_nodes() {
    section "Cleaning Data and IPC for all nodes"
    log_info "Stopping nodes before cleaning..."
    stop_nodes
    log_info "Removing data directories..."
    rm -rf "$DATA_DIR/A" "$DATA_DIR/B" "$DATA_DIR/C"
    rm -f "$IPC_A" "$IPC_B" "$IPC_C"
    log_success "Data and IPC for Node A, B, and C have been removed"
}

start_nodes() {
    section "Starting 3 Geth Nodes (Docker/Linux Mode)"
    
    # Cleanup any old processes (using pkill)
    pkill -9 geth 2>/dev/null || true
    rm -f "$IPC_A" "$IPC_B" "$IPC_C"
    
    # Node A (Passive, starts mining only during reorg)
    log_info "Starting Node A (Passive)..."
    $GETH_BIN --datadir "$DATA_DIR/A" --networkid 1 --port 30305 --authrpc.port 8552 \
      ...
      --syncmode full --gcmode archive --verbosity 3 > "$DATA_DIR/A/nodeA.log" 2>&1 &
    
    # Node B (Always Mining)
    log_info "Starting Node B (Mining)..."
    $GETH_BIN --datadir "$DATA_DIR/B" --networkid 1 --port 30306 --authrpc.port 8553 \
      --nat none --nodiscover --netrestrict 127.0.0.1/32 \
      --ipcpath "$IPC_B" \
      ...
      --unlock "0x573Df0Eb2F042bf92a897A5C37c144F717Ed1dd7" --password "$PASSWORD_FILE" \
      --syncmode full --gcmode archive --verbosity 3 > "$DATA_DIR/B/nodeB.log" 2>&1 &
    
    # Node C (Passive)
    log_info "Starting Node C (Passive)..."
    $GETH_BIN --datadir "$DATA_DIR/C" --networkid 1 --port 30307 --authrpc.port 8554 \
      --nat none --nodiscover --netrestrict 127.0.0.1/32 \
      --ipcpath "$IPC_C" \
      ...
      --allow-insecure-unlock \
      --unlock "0x573Df0Eb2F042bf92a897A5C37c144F717Ed1dd7" --password "$PASSWORD_FILE" \
      --syncmode full --gcmode archive --verbosity 3 > "$DATA_DIR/C/nodeC.log" 2>&1 &
...
attach_console() {
  ...
  # In Docker, we use standard file-based IPC path
...
menu() {
  ...
  echo -e "   1) ${GREEN}Run All Nodes${NC}"
  echo -e "   2) ${YELLOW}Init Genesis Block${NC}      (Required once)"
  echo -e "   3) ${GREEN}${BOLD}Faucet${NC}"
...
display_menu() {
  ...
  echo -e "${CYAN}                                  (DOCKER LINUX MODE)                          ${NC}"
  
  echo -e "\n  ${WHITE}Status: $(get_status)${NC} | ${DIM}Geth: 1.13.5-stable${NC} | ${DIM}Path: $DATA_DIR${NC}"
  menu
}
...
while true; do
  # read -t 30: auto-refresh dashboard every 30s when idle
  # This ensures 'docker attach' will show the full menu within 30 seconds
  read -t 30 -e -p "Select option: " choice
  rc=$?

  # Timeout (rc > 128) - refresh display (catches docker re-attach)
  if [ $rc -gt 128 ]; then
    clear
    display_menu
    continue
  ...
  case "$choice" in
    h|H|help) ;;
    1) start_nodes; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    2) init_genesis; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    3) faucet; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    4) show_block; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    5) list_peers; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    6) get_target_ipc; run_js_out "admin.nodeInfo.enode" "$TARGET_IPC"; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    7) reorg_simulation; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    8) get_target_ipc; run_js "miner.stop()" "$TARGET_IPC"; log_success "Mining stopped on Node $TARGET_NAME"; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    9) get_target_ipc; run_js "miner.start()" "$TARGET_IPC"; log_success "Mining started on Node $TARGET_NAME"; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    10) attach_console ;;
    11) clean_nodes; read -n 1 -s -r -p "[Press any key to continue...]" ;;
    c|C|clear) ;;
    0) exit 0 ;;
    *) log_warn "Invalid option"; sleep 1 ;;
  esac

  # Always redisplay full dashboard after every command
  clear
  display_menu
done
